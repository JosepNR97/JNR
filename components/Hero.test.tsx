...
describe('Hero CV download', () => {
  it.each(cases)(
    'uses the active $language document after the primary actions',
    ({ language, label }) => {
      ...
      const actionsContainer = download.parentElement;
      expect(actionsContainer).not.toBeNull();
      if (!actionsContainer) throw new Error('Hero actions were not rendered');
      const actions = within(actionsContainer).getAllByRole('link');
      ...
      expect(download).toHaveAttribute(
        'href',
        expect.stringContaining(`/assets/documents/${CV_FILENAMES[language]}`),
      );
